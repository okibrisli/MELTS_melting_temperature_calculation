"""
Batch MELTS melting-range calculator (v2, fixes the isobaric_crystallisation
timeout/oscillation problem seen in the first run).

What changed vs the first version, and why:
  1. TRACE_H2O_WT_PCT is added to every composition before it is sent to
     MELTS. alphaMELTS itself warned "Adding small amounts of H2O may
     improve the ability of MELTS to accurately reproduce the saturation of
     oxide minerals" - in the failed run, alkali-feldspar was being added
     and dropped from the assemblage dozens of times in a row (a classic
     MELTS solver flip-flop at a reaction boundary), which is exactly what
     this trace H2O is meant to damp out.
  2. The isobaric_crystallisation cooling path no longer scans all the way
     to a fixed 600 degC for every sample. It now scans from the sample's
     own liquidus down to (liquidus - MAX_UNDERCOOLING_C), floored at
     T_END_MIN_C. That is enough range to bracket a ceramic/glass solidus
     without wasting hours deep in the subsolidus region where the
     assemblage becomes unstable and slow to converge.
  3. Both MELTS calls are given an explicit timeout (the kwarg alphaMELTS
     itself told us about in the failed run) instead of relying on the
     5 minute internal default.
  4. Liquidus success and solidus success are tracked and reported
     separately. A sample that finds a liquidus but times out on the
     solidus sweep is reported as "solidus_incomplete" and keeps its
     liquidus value, instead of the whole row being thrown away as
     "failed".
  5. Per-sample wall-clock time is printed so you can see which
     compositions are the slow ones before committing to the full batch.
...
DT_C = 10.0                        # coarser than the first attempt (was 5.0), halves the step count
...
ISOBARIC_TIMEOUT_S = 900           # was defaulting to 300 s (5 min) and timing out, now 15 min
LIQUIDUS_TIMEOUT_S = 300
...
def clean_and_normalise(row, oxide_columns):
    ...
    # Add trace H2O to damp solver flip-flop, then renormalise back to 100%.
...
def find_solidus_from_path(xtal_result):
    ...
    if len(above_onset) == 0:
        return None, (
            ...
            f"scanned range -> solidus is below the scan window, lower MAX_UNDERCOOLING_C floor"
        )
...
def process_composition(sample_id, row, oxide_columns):
    ...
    try:
        ...
        # --- Stage 2: cooling path -> solidus (melting start) ---
...
def main():
    ...
    report_lines = [
        ...
        f"Timeouts: liquidus {LIQUIDUS_TIMEOUT_S}s, solidus sweep {ISOBARIC_TIMEOUT_S}s",
